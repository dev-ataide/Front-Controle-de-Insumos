...
export default function CreateClient({ isOpen, setModalOpen }) {
    ...
    async function handleRegisterCliente(event: FormEvent) {
        event.preventDefault();

        try {
            const data = new FormData();
            if (clienteName === '' || cpf === '' || email === '' || contato === '' || obs === '' || imageAvatar === null) {
                toast.error("Preencha todos os dados")
                return
            }

            data.append('file', imageAvatar);
            data.append('name', clienteName)
            ...
            await apiClient.post('/client', data)
            toast.success('Cliente cadastrado com Sucesso')
                    ...
                    <div className="sm:max-w-lg w-full p-10 bg-white rounded-xl z-10">
                        ...
                        <main className="flex items-center justify-center mt-5 bg-gray-100 font-sans">
                            <label className="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center rounded-xl border-2 border-dashed border-blue-400 bg-white p-6 text-center">
                                ...
                                {!avatarUrl && (
                                    <div>
                                        <h2 className="mt-4 text-xl font-medium text-gray-700 tracking-wide">Foto do seu Cliente</h2>
                        ...
                        <div className="max-w-2xl mx-auto bg-white">
                            ...
                            <form onSubmit={handleRegisterCliente} >
                                <div className="my-6">
                                    ...
                                    <input value={clienteName} onChange={(e) => setClientName(e.target.value)} type="fullname" id="fullname" className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Nome completo do cliente" required />
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" id="email" className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="emaildocliente@exemplo.com" required />
                                </div>
                                <div>
                                    <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Observações sobre o Cliente</label>
                                ...
                                <div className="flex items-start my-6">
                                    ...
                                    <label className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-400">Faça o Upload de um <a href="#" className="text-blue-600 hover:underline dark:text-blue-500">foto em boa qualidade do seu Cliente</a>.</label>
                                </div>
